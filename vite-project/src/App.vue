<template>
  <div class="container mx-auto flex flex-col">
    ...
    <ul class="mt-4 border-4 border-red-700">
      <li v-for="pokemon in pokemonList" :key="pokemon.id" class="flex items-center justify-between border-b border-gray-300 p-2 bg-red-700">
        <span class="text-lg text-white font-extrabold w-1/3">{{ pokemon.id }}</span>
        <span class="text-lg text-white font-extrabold w-1/3 text-center">{{ pokemon.name }}</span>
        <button @click="deletePokemon(pokemon.id)" class="text-lg text-white font-extrabold w-1/3 text-right">DELETE</button>
      </li>
    </ul>
  </div>
</template>
