...
export class AuthController {
  ...
  async login(@Body() credentials: { email: string, password: string }, @Res() res: any) {
    try {
      ...
      res.cookie('user', token, { maxAge: 60 * 60 * 24 * 1000, httpOnly: true }); // Set JWT token as cookie
    } catch (error) {
      return { url: '/login?error=true' }; // Redirect to login page with error
    }
  }
  ...
  async register(@Body() credentials: { email: string, password: string }) {
    try {
      ...
      return { url: '/signup?error=true' }; // Redirect to signup page with error
    }
  }
}
